...
function searchCountryForDeport(inputID, btnID){
	  $("#"+btnID).click(function(){
		     var countryName = getSearchParam(inputID);
			 $.get("SearchSelectAction_getCountryForDeport?&ts="
						+ new Date().getTime(), 
			    {countryName : countryName},
			    function(rdata) {
					var source = new Array();
...
function searchDetectionItem(inputID, btnID){
	$("#"+btnID).click(function(){
		 ...
		 if(detectionItem==''){alert("请输入检测项目关键字段");return;}
 ...
 * 查询企业
 */
function searchEntTwo(inputID, btnID,inputId2,inputId3){
	$("#"+btnID).click(function(){
		 var entName = getSearchParam(inputID);
		 $.get("SearchSelectAction_getEnt?&ts="
					+ new Date().getTime(), 
