...
public class IntendeUseExportAction  extends ActionSupport{
	...
	IntendedUseService service=new IntendedUseService();
	
	public String execute() throws Exception {

		String excelName = "预期用途数据";
		System.out.println("ac se "+data);
		//JSONObject rsData= ProductStatisticsService.getStaticResultForProduct(data, draw, 0, 0);
		//JSONObject object = JSONObject.fromObject(data);
		IntendedUseBean bean=new IntendedUseBean();
		bean.setUseName("");
		Map<Integer, Object> rsMap=  service.GetIntendedUse(bean, 1, 0, 0);
	...
	public <T> HSSFWorkbook generateExcel(String excelName,
		...
		try {
			int j = 0;
			Field fields[] = IntendedUseForExportBean.class.getDeclaredFields();
			for (; j < ja.size(); j++) {
