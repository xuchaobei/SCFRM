...
public class ProductStatisticsDao {
	...
	private static final Log log = LogFactory.getLog(ProductStatisticsDao.class);
	
	/**
	 * 根据统计条件获得货物批统计结果
	 * @return
	 */
	public static Map<Integer,Object> getStaticResultForProduct(int startYear, int startMonth, int endYear , int endMonth, String productCode, String productName,
		...
		List<ProductStatisticsDto> list = new ArrayList<ProductStatisticsDto>();
		Connection conn = null;
		CallableStatement proc = null;
		ResultSet rs = null;
		int recordsTotal = 0;
		String call = "{call Pro_GetStaticResultForProduct(?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
		try {
			...
			ProductStatisticsDto dto = null;
			while (rs.next()) {
				dto = RsToDtoUtil.tranRsToDto(rs, ProductStatisticsDto.class);
				list.add(dto);
			}
